# Mes Offres — Architecture finale
> Version 10.0 — Juin 2026
> Modèle : Tom Pearson (structure 2 portes + MRR) × Julien Musy / Empowerment (organique pur → scale) · SANS low ticket — la Porte 1 EST l'entrée (décision Juin 2026)
> Porte 1 : Deviens l'Entrepreneur™ (0→10K, organique pur) · Porte 2 : Construis la Marque™ (10K→100K, scale)
> Wiki : [[business-ascension]] · [[icp-cible]] · [[funnel-acquisition]] · [[tom-pearson]]

---

## VISION GLOBALE

Deux portes. Une marque. Une ascension.

**Business Ascension™** construit des entrepreneurs qui font du chiffre de façon prévisible en alignant leur business avec qui ils sont (identité) ET en construisant le système d'acquisition (business), en même temps.

- **Porte 1** = on construit les **fondations + l'écosystème organique** → premiers 10K€/mois réguliers + une communauté.
- **Porte 2** = on **scale** ce qui a été construit avec la porte 1 et on fait de l'acquisition payante, le recrutement et l'automatisation → 10K→100K€/mois.

Le différenciateur : identité + business **simultané**, chaque semaine. Pas l'un puis l'autre.

---

## LES 2 PILIERS — dans les deux portes

**I · Identité & Exécution** — Breathwork · Hypnose · PNL · Neurosciences · Reconditionnement. On enlève ce qui bloque l'exécution à la source. *Présent en Porte 1 ET Porte 2.*

**II · Business & Marketing** — Offre · Positionnement · Contenu qui pré-vend · Leads entrants · Vendre par message · Systèmes d'acquisition · BOS 6 pôles.

---

## PORTE 1 — Deviens l'Entrepreneur™
### 0 → 10K€/mois · Organique pur · Abonnement (MRR)

*Inspiration : Julien Musy "Funnel & Freedom" / Empowerment + MRR Tom Pearson.*

**Promesse :** Construire les fondations de ton business aligné + un écosystème de contenu organique qui crée une communauté et un CA récurrent — pour toi comme pour tes clients.

### Ce qu'on construit AVEC le client

**A. Les fondations (identité + offre)**
1. L'**offre** (irrésistible, calibrée marché)
2. L'**avatar** (ICP précis)
3. Le **positionnement** (catégorie à part)
4. Le **travail d'identité** : breathwork + hypnose + reconditionnement (débloquer l'exécution)

**B. Le système d'acquisition organique**
5. Un **lead magnet**
6. Des **systèmes de contenu par plateforme**, chacun avec son approche unique :
   - **LinkedIn** → méthode propre (autorité, écrit)
   - **Instagram** → méthode propre (short form, reels)
   - **YouTube** → méthode propre (long form)
   - **+ interconnexion des 3** → écosystème organique → **omniprésence**

**🎯 Objectif central : une COMMUNAUTÉ qui suit** → moteur de croissance + revenu récurrent (clients ET BA™).

### Format & Rémunération (MRR — prix public)
| Plan | Paiement unique | Mensualisé |
|---|---|---|
| **3 mois** | 997€ | 3 × 367€ |
| **6 mois** *(le plus choisi)* | 1 497€ | 6 × 297€ |
| **12 mois** *(meilleure valeur)* | 2 497€ | 12 × 247€ |

> Payable en 1 fois OU en 3 / 6 / 12 versements selon le plan · Accès immédiat · Protocole personnalisé sous 48h.
> *(Tarifs validés — Juin 2026.)*

**Garantie :** roadmap construite + premier résultat mesurable à 30 jours, sinon remboursement intégral.

---

## PORTE 2 — Construis la Marque™
### 10K → 100K€/mois · Scale · Mentorship 12 mois · Candidature

*Inspiration : Julien Musy "Millionnaire Lion / Infinity" + mentorship Tom Pearson.*

**Promesse :** Scaler ce qui a été construit en Porte 1 — amplifier et accélérer la croissance avec l'acquisition payante, sortir de l'opérationnel, et automatiser.

### Ce qu'on ajoute (au-dessus de tout l'organique, qui continue)

1. **Acquisition payante** : ADS + webinaire + challenge + VSL → amplifie et accélère
2. **Recrutement** → recruter les bonnes personnes pour **sortir de l'opérationnel**
3. **Automatisation** des tâches récurrentes
4. **Identité / mindset (continue)** : hypnose, breathwork, reconditionnement à un niveau supérieur

### Format & Rémunération
- **Parcours unique · Engagement 12 mois** (plus de niveaux multiples)
- **Pas de prix public** · prix donné **uniquement en appel** · valeur définie ensemble plus tard
- **Candidature uniquement** · conversation 1-to-1 avec Chris

---

## L'ASCENSION COMPLÈTE (funnel)

```
LEAD MAGNET  →  capture + nurture
        ↓
CONTENU ORGANIQUE (LinkedIn · Instagram · YouTube interconnectés)
        ↓  → COMMUNAUTÉ + leads entrants + vendre par message
        ↓
PORTE 1 — Deviens l'Entrepreneur™  (0→10K · MRR · prix public)
        ↓
PORTE 2 — Construis la Marque™  (10K→100K · scale · 12 mois · prix par appel)
        ↓
SCALE ASCENSION  (continuité clients avancés)
```

---

## NOMENCLATURE OFFICIELLE

| Terme | Statut |
|---|---|
| **Deviens l'Entrepreneur™** | ✅ Porte 1 — organique — 0→10K |
| **Construis la Marque™** | ✅ Porte 2 — scale — 10K→100K |
| **Business Ascension™** | ✅ La marque |
| **Scale Ascension** | ✅ Continuité clients avancés |
| **Audit offert** | ✅ Nom public de l'appel |
| **Diagnostic Ascension** | ⚠️ Interne uniquement |

---

*Business Ascension™ — Mes Offres v10.0 — Juin 2026*
*Modèle : Tom Pearson × Julien Musy (Empowerment) adapté BA™*
