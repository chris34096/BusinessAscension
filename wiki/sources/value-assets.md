---
wiki_type: source
name: Value Assets BA™ — Catalogue
slug: value-assets
created: 2026-05-23
updated: 2026-05-23
confidence: high
sources:
  - 01-MARKETING/value-assets/value-asset-01-vraie-raison-stagnation.md
  - 01-MARKETING/value-assets/value-asset-02-formations-ne-suffisent-pas.md
  - 01-MARKETING/value-assets/value-asset-03-mois-12000-mois-zero.md
  - 01-MARKETING/value-assets/value-asset-04-business-operating-system-6-poles.md
  - 01-MARKETING/value-assets/value-asset-05-don-quichotte-entrepreneur.md
  - 01-MARKETING/value-assets/value-asset-06-switch-chiffre-affaires.md
related:
  - [[dents-de-scie]]
  - [[inner-architecture-protocol]]
  - [[icp-cible]]
  - [[bos-6-poles]]
  - [[christofer-perez]]
  - [[business-ascension]]
  - [[don-quichotte-archetype]]
---

# Value Assets BA™ — Catalogue
6 scripts vidéo YouTube longs (10-20 min) avec repurposing Instagram + LinkedIn. Chaque asset externalise un angle clé du positionnement BA™ et se termine par CTA Audit offert.

## VA01 — La vraie raison pour laquelle tu stagnes malgré l'effort
- **Format :** Diagnostic YouTube — 12-15 min
- **Angle :** Le mécanisme réel derrière la stagnation — les 4 goulots + les deux côtés non traités en même temps
- **Concepts :** [[dents-de-scie]] · [[inner-architecture-protocol]] · [[icp-cible]]
- **Repurposing :** 4 Reels · 2 LinkedIn · 3 Shorts
- **Fichier :** `01-MARKETING/value-assets/value-asset-01-vraie-raison-stagnation.md`

## VA02 — Pourquoi tu peux faire 10 formations et ne pas avancer d'un euro
- **Format :** Vérité sur le Marché — 10-12 min
- **Angle :** Problème systémique du coaching business en ligne — outil sans architecte, niveau cognitif sans désactivation SN
- **Concepts :** [[icp-cible]] · [[business-ascension]]
- **Repurposing :** 4 Reels · 2 LinkedIn · 3 Shorts
- **Fichier :** `01-MARKETING/value-assets/value-asset-02-formations-ne-suffisent-pas.md`

## VA03 — Des mois à 12 000€. Des mois à zéro.
- **Format :** Story Personnelle — 12-15 min
- **Angle :** Histoire réelle de Chris — crédibilité via vulnérabilité, non performative
- **Concepts :** [[christofer-perez]] · [[dents-de-scie]]
- **Repurposing :** 3 Reels · 2 LinkedIn · 2 Shorts
- **Fichier :** `01-MARKETING/value-assets/value-asset-03-mois-12000-mois-zero.md`

## VA04 — Le Business Operating System : les 6 pôles
- **Format :** Méthode — 15-20 min
- **Angle :** Introduction du livrable concret — ce que le client repart avec, démontre la profondeur
- **Concepts :** [[bos-6-poles]] · [[inner-architecture-protocol]]
- **Repurposing :** 4 Reels · 2 LinkedIn · 3 Shorts
- **Fichier :** `01-MARKETING/value-assets/value-asset-04-business-operating-system-6-poles.md`

## VA05 — Don Quichotte entrepreneur
- **Format :** Diagnostic — 12-15 min
- **Angle :** Nommer l'archétype avec précision — reconnaissance immédiate chez le Don Quichotte
- **Concepts :** [[icp-cible]] · [[don-quichotte-archetype]]
- **Repurposing :** 4 Reels · 2 LinkedIn · 3 Shorts
- **Fichier :** `01-MARKETING/value-assets/value-asset-05-don-quichotte-entrepreneur.md`

## VA06 — Comment je peux t'aider à faire un switch
- **Format :** Vidéo personnelle — connexion + mécanisme
- **Angle :** Explication concrète du modèle BA™ — ce que Chris fait exactement et pourquoi c'est différent
- **Concepts :** [[inner-architecture-protocol]] · [[business-ascension]]
- **Repurposing :** 2 Reels · 1 carousel LinkedIn · 1 post LinkedIn
- **Fichier :** `01-MARKETING/value-assets/value-asset-06-switch-chiffre-affaires.md`

## Observations

- VA03 est le plus fort pour la crédibilité — histoire non performative, validée par Chris lui-même
- VA05 est le seul à nommer explicitement l'archétype Don Quichotte — crée la reconnaissance la plus directe
- VA04 est le seul à montrer les livrables concrets — réduit la méfiance de la cible face aux promesses floues
- Les 6 assets couvrent tous les piliers du calendrier éditorial : Diagnostic (VA01, VA05), Vérité Marché (VA02), Méthode (VA04), Histoire (VA03, VA06)
