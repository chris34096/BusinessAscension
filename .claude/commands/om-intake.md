---
description: "Process all unread meeting notes in 08-SECOND-CERVEAU/work/meetings/ — reads each file, classifies content, routes to the right vault notes, then clears the inbox."
---

# Meeting Intake

Scans `08-SECOND-CERVEAU/work/meetings/` for unprocessed meeting exports and routes everything to the correct vault locations automatically. No arguments needed — just drop files and run.
...
Drop any exported or raw meeting note into `08-SECOND-CERVEAU/work/meetings/` first. Run this command after. Naming convention: `YYYY-MM-DD <Topic or Person>.md`.

## Workflow

### 1. Scan the Inbox

List all `.md` files in `08-SECOND-CERVEAU/work/meetings/` excluding `README.md`. If the folder is empty, say so and stop.
...
|---|---|
| 1:1 with a specific person | Create `08-SECOND-CERVEAU/work/1-1/<Person> YYYY-MM-DD.md` using 1:1 note structure |
| Project status update | Append to relevant `08-SECOND-CERVEAU/work/active/<Project>.md` |
| New project or initiative not in vault | Create `08-SECOND-CERVEAU/work/active/<Project>.md` |
| Decision reached | Create Decision Record in `08-SECOND-CERVEAU/work/` + add to Decisions Log in `08-SECOND-CERVEAU/work/Index.md` |
| Action item / open task | Append as `- [ ]` to the relevant work note |
| Win or recognition | Add to `08-SECOND-CERVEAU/perf/Brag Doc.md` with link to source note |
| New person mentioned not in vault | Create stub in `08-SECOND-CERVEAU/org/people/<Name>.md` |
| Blocker identified | Append to relevant `08-SECOND-CERVEAU/work/active/` note under `## Blockers` or `## Open / Next Steps` |
...
- If a person was mentioned, link to their `08-SECOND-CERVEAU/org/people/` note from the work note and vice versa
