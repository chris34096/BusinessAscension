...
- `--dry-run` — generate the migration plan without executing it (saves to `08-SECOND-CERVEAU/thinking/`)
...
4. Check for `vault-manifest.json` in the source:
   ...
   - **Not found**: run version fingerprint detection from the target's `vault-manifest.json`:
     - Check for `claude/Memories.md` without `08-SECOND-CERVEAU/brain/` → v1
     - Check for `08-SECOND-CERVEAU/brain/` + `08-SECOND-CERVEAU/bases/` without `.claude/agents/` → v2
...
| `08-SECOND-CERVEAU/bases/**` | **MERGE** | Keep template Bases, preserve user-added Bases, log conflicts for review |
...
If `08-SECOND-CERVEAU/brain/Migration Log.md` exists in the target vault, read it and extract the recorded hashes. Each log entry stores both a **source hash** (original file content) and a **final hash** (transformed content written to target).
...
Files found: 87
  ...
  CLASSIFY:  5 uncategorized → 08-SECOND-CERVEAU/thinking/migrate-review/
...
|---|--------|--------|--------|------------|
| 1 | 08-SECOND-CERVEAU/org/people/Jane.md | 08-SECOND-CERVEAU/org/people/Jane.md | COPY | +description |
| 2 | 08-SECOND-CERVEAU/brain/Memories.md | 08-SECOND-CERVEAU/brain/Memories.md | REPLACE | — |
| 3 | daily/2026-03-15.md | 08-SECOND-CERVEAU/work/1-1/... | CLASSIFY+COPY | +quarter, +tags |
```
...
**For `--dry-run`**: write the plan to `08-SECOND-CERVEAU/thinking/vault-upgrade-plan-YYYY-MM-DD.md` and stop.
...
Migration complete.
  ...
  ✓ Indexes rebuilt (08-SECOND-CERVEAU/work/Index.md, People & Context, Brag Doc, Memories)
  ✓ .obsidian/ config copied

  ⚠ 3 orphan notes (no inbound links) — run /om-vault-audit to fix
  ⚠ 5 files in 08-SECOND-CERVEAU/thinking/migrate-review/ need manual sorting

  Migration log: 08-SECOND-CERVEAU/brain/Migration Log.md
```

Offer next steps:
- "Run `/om-vault-audit` for a full structural check"
- "Review `08-SECOND-CERVEAU/thinking/migrate-review/` for uncategorized files"
- "Check `08-SECOND-CERVEAU/brain/Migration Log.md` for the full receipt and CLAUDE.md diff"
...
- This command creates `08-SECOND-CERVEAU/thinking/migrate-review/` only if there are uncategorized files. Clean it up after manual sorting.
