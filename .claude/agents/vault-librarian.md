...
3. **Frontmatter Validation**: Glob all `.md` files in `08-SECOND-CERVEAU/work/`, `08-SECOND-CERVEAU/org/`, `08-SECOND-CERVEAU/perf/`, `08-SECOND-CERVEAU/brain/`, `reference/`. Check each has:
   - `tags` (non-empty)
   - `date`
   - `description` (~150 chars)
   - Type-specific required fields (incidents need `ticket`, `severity`, `role`; work notes in recent quarters need `quarter`)

4. **Stale Active Notes**: Check `08-SECOND-CERVEAU/work/active/` for notes with `status: completed` or not modified in 60+ days. These should be archived to `08-SECOND-CERVEAU/work/archive/YYYY/`.

5. **Index Consistency**: Read `08-SECOND-CERVEAU/work/Index.md` and verify all notes listed under "Active Projects" actually exist in `08-SECOND-CERVEAU/work/active/`. Flag any that are missing or archived.

6. **Cross-Link Quality**: For notes in `08-SECOND-CERVEAU/work/active/` and `08-SECOND-CERVEAU/work/incidents/`, check they link to at least one person (`08-SECOND-CERVEAU/org/people/`), one project or team reference, and relevant competencies.

## Output

Write the maintenance report to `08-SECOND-CERVEAU/thinking/vault-audit-YYYY-MM-DD.md` with:
