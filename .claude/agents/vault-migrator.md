...
| **Inbox** | Has `Inbox/`, `Unsorted/`, `_inbox/` | Unprocessed notes — classify by content or send to `08-SECOND-CERVEAU/thinking/migrate-review/` |
...
|--------|---------------|--------|
| File in folder named `people/`, `contacts/`, `team-members/` | Person | `08-SECOND-CERVEAU/org/people/` |
| File in folder named `teams/`, `squads/`, `groups/` | Team | `08-SECOND-CERVEAU/org/teams/` |
| File in folder named `incidents/`, `postmortems/`, `outages/` | Incident | `08-SECOND-CERVEAU/work/incidents/` |
| File in folder named `1-1/`, `one-on-ones/`, `meetings/` | 1:1 | `08-SECOND-CERVEAU/work/1-1/` |
| File in folder named `archive/`, `completed/`, `done/` | Archived work | `08-SECOND-CERVEAU/work/archive/YYYY/` |
| File in folder named `projects/`, `active/`, `wip/` | Active work | `08-SECOND-CERVEAU/work/active/` |
| File in folder named `brag/`, `wins/`, `achievements/` | Brag | `08-SECOND-CERVEAU/perf/brag/` |
| File in folder named `evidence/`, `pr-scans/` | Evidence | `08-SECOND-CERVEAU/perf/evidence/` |
| File in folder named `reference/`, `docs/`, `architecture/` | Reference | `reference/` |
| File in folder named `08-SECOND-CERVEAU/templates/` | Template | SKIP (target has its own) |

**Common Obsidian vault patterns:**

| Signal | Classification | Target |
|--------|---------------|--------|
| PARA: `Projects/` | Active work | `08-SECOND-CERVEAU/work/active/` |
| PARA: `Areas/` | Ongoing responsibilities — classify by content | Varies (08-SECOND-CERVEAU/work/active/, org/, brain/) |
| PARA: `Resources/` | Reference material | `reference/` |
| PARA: `Archive/` | Completed or inactive | `08-SECOND-CERVEAU/work/archive/YYYY/` |
| Zettelkasten: `Permanent/` or `Evergreen/` | Mature notes — classify by content | Varies |
| Zettelkasten: `Fleeting/` or `Scratch/` | Scratchpad | SKIP or `08-SECOND-CERVEAU/thinking/` |
| Zettelkasten: `Literature/` or `Sources/` | Reference | `reference/` |
| Daily notes: `daily/`, `journal/`, `Daily Notes/` | Journal entries — classify by content | Varies (08-SECOND-CERVEAU/work/1-1/, thinking/) |
| Inbox: `Inbox/`, `Unsorted/`, `_inbox/` | Unprocessed | `08-SECOND-CERVEAU/thinking/migrate-review/` |
...
| Signal | Classification | Target |
|--------|---------------|--------|
| `<Name> YYYY-MM-DD.md` | 1:1 meeting note | `08-SECOND-CERVEAU/work/1-1/` |
| `YYYY-MM-DD.md` (date only) | Daily note — classify by content | Varies |
| `Q[1-4] YYYY.md` in brag-like folder | Brag | `08-SECOND-CERVEAU/perf/brag/` |
| `<Person> PRs - *.md` | PR evidence | `08-SECOND-CERVEAU/perf/evidence/` |
...
|--------|---------------|
| `tags` contains `person` or `people` | Person → `08-SECOND-CERVEAU/org/people/` |
| `tags` contains `team` | Team → `08-SECOND-CERVEAU/org/teams/` |
| `tags` contains `incident` or has `severity`/`ticket` field | Incident → `08-SECOND-CERVEAU/work/incidents/` |
| `tags` contains `work-note` or `project` and `status: active` | Active work → `08-SECOND-CERVEAU/work/active/` |
| `tags` contains `work-note` or `project` and `status: completed`/`done` | Archived work → `08-SECOND-CERVEAU/work/archive/YYYY/` |
| `tags` contains `competency` or `skill` | Competency → `08-SECOND-CERVEAU/perf/competencies/` |
| `tags` contains `brain` or `north-star` or `goals` | Brain note → `08-SECOND-CERVEAU/brain/` |
| `tags` contains `decision` or `adr` | Decision → `08-SECOND-CERVEAU/work/active/` or `08-SECOND-CERVEAU/work/archive/YYYY/` |
| `tags` contains `meeting` or `1-on-1` or `1:1` | 1:1 → `08-SECOND-CERVEAU/work/1-1/` |
| `tags` contains `review` or has `cycle` field | Review artifact → `08-SECOND-CERVEAU/perf/<cycle>/` |
| Has `person` field (not tag) pointing to a name | Evidence → `08-SECOND-CERVEAU/perf/evidence/` |

**Inline signals (for vaults with no YAML frontmatter):**

| Signal | Classification |
|--------|---------------|
| Inline tags `#meeting`, `#1on1`, `#standup` in first 10 lines | 1:1 → `08-SECOND-CERVEAU/work/1-1/` |
| Inline tags `#incident`, `#postmortem`, `#outage` | Incident → `08-SECOND-CERVEAU/work/incidents/` |
| Inline tags `#project`, `#feature`, `#epic`, `#sprint` | Work note → `08-SECOND-CERVEAU/work/active/` |
| Inline tags `#person`, `#teammate`, `#colleague` | Person → `08-SECOND-CERVEAU/org/people/` |
...
| Signal | Classification |
|--------|---------------|
| Discusses a specific person's role, working style, or interactions | Person → `08-SECOND-CERVEAU/org/people/` |
| Describes a project, feature, or technical work with deliverables | Work note → `08-SECOND-CERVEAU/work/active/` or `08-SECOND-CERVEAU/work/archive/YYYY/` |
| Contains meeting notes with attendees, discussion points, action items | 1:1 → `08-SECOND-CERVEAU/work/1-1/` |
| References an incident, outage, or production issue with investigation | Incident → `08-SECOND-CERVEAU/work/incidents/` |
| Contains architectural diagrams, API docs, flow descriptions | Reference → `reference/` |
| Discusses goals, career plans, or focus areas | Brain → `08-SECOND-CERVEAU/brain/` |
...
Files that match nothing after all tiers get classified as `uncategorized` with target `08-SECOND-CERVEAU/thinking/migrate-review/`. Tag them with `migrate-review` in frontmatter so the user can sort them manually.
...
2. **Transform frontmatter**:
   ...
   - Add missing required fields per the manifest's `frontmatter_required`:
     ...
     - `status`: `active` if going to `08-SECOND-CERVEAU/work/active/`, `completed` if going to `08-SECOND-CERVEAU/work/archive/`
...
**Scaffold files** (08-SECOND-CERVEAU/brain/Memories.md, 08-SECOND-CERVEAU/work/Index.md, etc.):
...
- `08-SECOND-CERVEAU/work/Index.md`: verify work note links, add missing ones grouped by status
- `08-SECOND-CERVEAU/brain/Memories.md`: verify topic note links, add any new brain/ notes
- `08-SECOND-CERVEAU/org/People & Context.md`: verify people and team links, add missing ones
- `08-SECOND-CERVEAU/perf/Brag Doc.md`: verify quarter note links and evidence links

### Step 5: Write Migration Log

Write `08-SECOND-CERVEAU/brain/Migration Log.md` with:
...
- Any files sent to `08-SECOND-CERVEAU/thinking/migrate-review/` for manual sorting
...
- Move `claude/` contents to `08-SECOND-CERVEAU/brain/` (rename `claude/Memories.md` → `08-SECOND-CERVEAU/brain/Memories.md`, etc.)
- If `claude/Memories.md` is a monolith (single file with multiple topic sections), split into separate topic files (`08-SECOND-CERVEAU/brain/Key Decisions.md`, `08-SECOND-CERVEAU/brain/Patterns.md`, `08-SECOND-CERVEAU/brain/Gotchas.md`)
- Move flat `08-SECOND-CERVEAU/work/*.md` notes into `08-SECOND-CERVEAU/work/active/` or `08-SECOND-CERVEAU/work/archive/YYYY/` based on status
...
- **Idempotency.** Check content hashes against `08-SECOND-CERVEAU/brain/Migration Log.md` before processing. Skip unchanged files, flag changed ones.
