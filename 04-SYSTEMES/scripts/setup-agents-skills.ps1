...
# Auto-élévation : relance en admin si nécessaire.
#
# USAGE : clic droit > Exécuter avec PowerShell  (ou : powershell -File setup-agents-skills.ps1)

# --- Auto-élévation admin ---
$isAdmin = ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)
if (-not $isAdmin) {
    Write-Host "Élévation admin nécessaire — relance..."
    Start-Process powershell.exe "-NoProfile -ExecutionPolicy Bypass -File `"$PSCommandPath`"" -Verb RunAs
    exit
}

$root = "E:\Document\VS CODE\Marketing\Business Ascension™\04-SYSTEMES\scripts"
