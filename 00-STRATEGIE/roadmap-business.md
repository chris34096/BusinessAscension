# Roadmap Business — Business Ascension™
> Version 2.0 — Juin 2026 — Les DEUX portes trackées (P1 MRR intégrée à chaque phase · low ticket supprimé · P1 = entrée du funnel)
> Horizon : 18 mois (Mai 2026 → Octobre 2027)
> Revue : à chaque session de clôture (fin de trimestre).
> Ce document traduit la vision en plan d'exécution trimestriel.
> Wiki : [[business-ascension]] · [[bos-6-poles]]

---

## CE QUE CE DOCUMENT EST — ET N'EST PAS

**Ce que c'est :** un plan d'exécution concret. Ce qu'on construit, dans quel ordre, avec quels jalons pour valider qu'on passe à l'étape suivante.

**Ce que ce n'est pas :** un planning figé. Chaque trimestre se termine par une session de clôture. Si les chiffres ou les signaux disent d'ajuster — on ajuste. La roadmap sert de cap, pas de cage.

**Principe de séquençage :**
> Un seul goulot à la fois. On ne scale pas l'acquisition avant que la conversion soit solide. On ne construit pas Scale Ascension avant que Construis la Marque™ ait ses preuves. Chaque phase a une condition d'entrée et une condition de sortie.

---

## VUE D'ENSEMBLE — LES 4 PHASES

```
Rappel modèle v10 : Porte 2 (Construis la Marque™) = prix annoncé UNIQUEMENT en appel.
Les montants ci-dessous sont des ANCRAGES INTERNES — jamais affichés publiquement.
Porte 1 (Deviens l'Entrepreneur™) = MRR public : 997 / 1 497 / 2 497 € (3/6/12 mois).

PHASE 1 — BÊTA          Q2 2026 (Mai → Juillet)
Poser les fondations. Signer les 3 premiers clients Porte 2. Documenter les preuves.
Lancer la Porte 1 (MRR) — c'est l'entrée du funnel.
Ancrage interne Porte 2 : 4 997 €

PHASE 2 — STANDARD      Q3-Q4 2026 (Août → Décembre)
Monter l'ancrage. Scaler l'acquisition organique. 5 à 8 clients Porte 2 actifs + communauté Porte 1 qui grandit.
Ancrage interne Porte 2 : 7 000 €

PHASE 3 — SCALE         Q1-Q2 2027 (Janvier → Juin)
Scale Ascension lancé. Acquisition payante BA™ activée. Systèmes renforcés.
Ancrage interne Porte 2 : 9 000 €

PHASE 4 — EMPIRE FRANCE Q3-Q4 2027 (Juillet → Décembre)
1er séminaire. Position de référence. Préparation expansion.
Ancrage interne Porte 2 : 9 000-12 000 €
```

---

## PHASE 1 — BÊTA
### Q2 2026 (Mai → Juillet 2026)

**Objectif unique :** prouver que le modèle fonctionne. Pas scaler — documenter.

**Condition d'entrée :** BOS complet ✅
**Condition de sortie :** 3 clients Construis la Marque™ signés + 3 témoignages avec résultats chiffrés + **Porte 1 vendable (paiements branchés) avec 5+ membres MRR actifs**

---

#### Ce qu'on construit ce trimestre

| Priorité | Action | Porte | Pôle | Deadline |
|---|---|---|---|---|
| 1 | Publier 5 Reels/semaine — 10 semaines consécutives | Les 2 | Marketing | Continu |
| 2 | Lancer la prospection DM (Instagram + LinkedIn) — 10 DMs/jour max | Les 2 | Marketing | Semaine 1 |
| 3 | Réaliser 3 Audits offerts/semaine | Les 2 | Sales | Semaine 2 |
| 4 | **Brancher les paiements Porte 1** (Stripe/GHL sur les 3 plans — les boutons sont en href="#") | P1 | Systèmes | Semaine 1 |
| 5 | **Ouvrir l'espace communauté Porte 1** (GHL/Skool/Circle — trancher l'outil) | P1 | Delivery | Semaine 2 |
| 6 | Signer le 1er client Construis la Marque™ | P2 | Sales | Semaine 4-6 |
| 7 | **Signer les 5 premiers membres Porte 1** (DM tièdes + audience existante) | P1 | Sales | Semaine 4-8 |
| 8 | Délivrer le 1er sprint P2 avec DoD validée + 1er protocole 48h P1 | Les 2 | Delivery | Semaine 6-8 |
| 9 | Signer les clients P2 n°2 et 3 | P2 | Sales | Semaine 6-10 |
| 10 | Collecter les 3 premiers témoignages écrits | Les 2 | Delivery | Semaine 10-12 |

#### Ce qu'on ne fait PAS ce trimestre

- Publicité payante (zéro budget ads — organique uniquement)
- Création de nouvelles offres
- Expansion LinkedIn au-delà du minimum (3 posts/semaine)
- YouTube (optionnel — si le temps le permet, sinon après)

#### Jalons de validation Q2

| Jalon | Semaine cible | Validé ? |
|---|---|---|
| Stack technologique actif (Stripe + Calendly + Notion + Zoom) | S2 | |
| **Paiements Porte 1 branchés (les 3 plans achetables)** | S1-2 | |
| Premier Audit offert réalisé | S2 | |
| **Premier membre Porte 1 payant (MRR > 0)** | S3-4 | |
| Premier contrat Porte 2 signé | S4-6 | |
| **5 membres Porte 1 actifs · MRR ≥ 1 500€/mois** | S8 | |
| 3 clients Porte 2 actifs simultanément | S8 | |
| Premier Sprint Review avec DoD validée | S8 | |
| 3 témoignages collectés | S12 | |

---

## PHASE 2 — STANDARD
### Q3-Q4 2026 (Août → Décembre 2026)

**Objectif :** monter l'ancrage prix Porte 2 à 7 000 € (toujours annoncé en appel), scaler l'acquisition, stabiliser le delivery avec 5 à 8 clients actifs — et faire grandir la communauté Porte 1 vers 20-30 membres (MRR ≥ 6 000€/mois).

**Condition d'entrée :** 3 témoignages documentés + BOS delivery validé sur 3 clients + Porte 1 active (5+ membres)
**Condition de sortie :** 5+ clients Porte 2 actifs simultanément (ancrage 7 000 €) + taux de conversion ≥ 33% + **20+ membres Porte 1 · churn < 10%/mois**

---

#### Ce qu'on construit Q3 (Août → Octobre)

| Priorité | Action | Pôle |
|---|---|---|
| 1 | Monter l'ancrage Porte 2 (annoncé en appel) à 7 000 € | Sales |
| 2 | Augmenter le rythme d'Audits offerts à 5/semaine | Sales |
| 3 | Lancer YouTube — 2 vidéos/mois minimum | Marketing |
| 4 | Tester les premières ads (500-1 000 €/mois) si CA > 10k | Marketing |
| 5 | Affiner le script-audit-offert.md selon les objections récurrentes | Sales |
| 6 | Documenter 2 études de cas complètes (before/after) | Delivery |
| 7 | Viser 5 clients actifs simultanément | Delivery |

#### Ce qu'on construit Q4 (Novembre → Décembre)

| Priorité | Action | Pôle |
|---|---|---|
| 1 | Préparer le lancement Scale Ascension (si 5 clients Construis la Marque™ complétés) | Delivery |
| 2 | Monter les ads à 1 000-2 000 €/mois si ROI validé | Marketing |
| 3 | Produire les premières études de cas vidéo | Marketing |
| 4 | Retravailler le BOS si des frictions sont apparues | Systèmes |
| 5 | Fixer les OKRs 2027 | Stratégie |

#### Jalons de validation Phase Standard

| Jalon | Horizon | Validé ? |
|---|---|---|
| Premier client Porte 2 signé à 7 000 € | Août 2026 | |
| 5 clients actifs simultanément | Octobre 2026 | |
| Taux de conversion Audit → client ≥ 33% | Octobre 2026 | |
| CA mensuel ≥ 14 000 € | Octobre 2026 | |
| 20+ membres Porte 1 · MRR >= 6 000 EUR/mois | Décembre 2026 | |
| 5 témoignages documentés avec résultats chiffrés | Décembre 2026 | |
| 2 études de cas complètes | Décembre 2026 | |

---

## PHASE 3 — SCALE
### Q1-Q2 2027 (Janvier → Juin 2027)

**Objectif :** lancer Scale Ascension, monter l'ancrage Porte 2 à 9 000 €, atteindre 30 000 €/mois de CA (dont 10 000 €+ de MRR Porte 1 — 40-50 membres).

**Condition d'entrée :** 5 clients Construis la Marque™ complétés + taux de réussite documenté ≥ 80%
**Condition de sortie :** Scale Ascension actif (3+ clients) + CA mensuel stable > 25 000 €

---

#### Ce qu'on construit Q1 2027 (Janvier → Mars)

| Priorité | Action | Pôle |
|---|---|---|
| 1 | Lancer Scale Ascension — 3 à 5 premiers profils | Delivery |
| 2 | monter l'ancrage Porte 2 à 9 000 € | Sales |
| 3 | Scaler le contenu YouTube (4 vidéos/mois) | Marketing |
| 4 | Tester les premières ads anglophone si budget le permet | Marketing |
| 5 | Documenter la méthode Construis la Marque™ (en vue de la certification future) | Systèmes |

#### Ce qu'on construit Q2 2027 (Avril → Juin)

| Priorité | Action | Pôle |
|---|---|---|
| 1 | Préparer le 1er séminaire présentiel (date + format + ville) | Business |
| 2 | Construire la liste d'attente séminaire via contenu | Marketing |
| 3 | Affiner Scale Ascension selon le feedback des 1ers clients | Delivery |
| 4 | Recruter un premier appui (CSM/coach delivery pour reprendre le 1:1 P2 — décision 2026-06-11 — ou assistant ops) si CA > 25k | RH |

#### Jalons de validation Phase Scale

| Jalon | Horizon | Validé ? |
|---|---|---|
| Scale Ascension ouvert et 3 premiers clients | Janvier 2027 | |
| Ancrage Porte 2 à 9 000 € avec premier client signé | Février 2027 | |
| CA mensuel ≥ 25 000 € (dont MRR Porte 1 ≥ 8 000 €) | Mars 2027 | |
| 10 témoignages documentés | Mars 2027 | |
| 1er séminaire annoncé avec liste d'attente | Juin 2027 | |

---

## PHASE 4 — EMPIRE FRANCE
### Q3-Q4 2027 (Juillet → Décembre 2027)

**Objectif :** 1er séminaire présentiel, position de référence en France, préparation expansion anglophone.

**Condition d'entrée :** Scale Ascension actif + CA mensuel stable > 25 000 € + 20 témoignages documentés
**Condition de sortie :** 1er séminaire réalisé + présence YouTube anglophone lancée

---

#### Ce qu'on construit

| Priorité | Action | Pôle |
|---|---|---|
| 1 | Organiser et réaliser le 1er séminaire présentiel (France) | Business |
| 2 | Lancer le contenu YouTube anglophone | Marketing |
| 3 | Monter l'ancrage Porte 2 à 10 000-12 000 € selon la demande | Sales |
| 4 | Préparer la certification du mécanisme BA™ en 8 étapes | Systèmes |
| 5 | Préparer la retraite immersive 2028 | Business |

---

## RÈGLES DE PASSAGE DE PHASE

Ces règles sont non négociables. On ne passe à la phase suivante sans avoir validé les conditions de sortie.

### Bêta → Standard
- [ ] 3 clients Construis la Marque™ complétés (ou en cours avancé)
- [ ] 3 témoignages écrits avec résultats concrets (CA, clarté, systèmes)
- [ ] Score satisfaction moyen ≥ 8/10
- [ ] BOS delivery validé (les SOPs tiennent sur 3 clients simultanés)

### Standard → Scale
- [ ] 5 clients Construis la Marque™ complétés
- [ ] Taux de conversion Audit → client ≥ 33% sur 10 appels
- [ ] CA mensuel ≥ 14 000 € pendant 2 mois consécutifs
- [ ] 2 études de cas documentées avec résultats chiffrés

### Scale → Empire France
- [ ] Scale Ascension actif avec 3+ clients
- [ ] CA mensuel ≥ 25 000 € pendant 2 mois consécutifs
- [ ] 20+ témoignages documentés
- [ ] Playbook delivery reproductible (pas uniquement dans la tête de Chris)

---

## SIGNAUX D'ALERTE — QUAND RECALIBRER

Ces situations déclenchent une révision de la roadmap avant la prochaine session de clôture.

| Signal | Action |
|---|---|
| Zéro client signé en 6 semaines | Analyser le goulot : acquisition ou conversion ? Retravailler en priorité |
| Taux de conversion < 20% sur 5 appels consécutifs | Retravailler le script-audit-offert.md + noter les objections récurrentes |
| Score satisfaction client < 7 | Rétrospective dédiée — comprendre avant de continuer |
| CA ne dépasse pas 5 000 €/mois après 2 mois en Standard | Retourner en mode bêta sur l'acquisition |
| Chris est le goulot du delivery (épuisement) | Documenter + déléguer avant d'accepter de nouveaux clients |

---

## LIEN AVEC LES AUTRES DOCUMENTS

| Document | Rôle par rapport à cette roadmap |
|---|---|
| [vision-empire.md](vision-empire.md) | Le cap long terme — cette roadmap en est l'opérationalisation |
| [okrs.md](../05-FINANCE/okrs.md) | Les OKRs trimestriels — déclinés depuis cette roadmap |
| [kpis-dashboard.md](../05-FINANCE/kpis-dashboard.md) | Les indicateurs hebdomadaires — signalent si la roadmap est tenue |
| [budget-previsionnel.md](../05-FINANCE/budget-previsionnel.md) | Les hypothèses financières — alignées avec les phases |
| [script-audit-offert.md](../02-SALES/script-audit-offert.md) | L'outil clé de la phase bêta — à itérer à chaque phase |

---

## PROCHAINE PI REVIEW

**Date :** Fin juillet 2026 (semaine 12 du Q2)

**Questions à se poser :**
1. Les conditions de sortie de la Phase Bêta sont-elles remplies ?
2. Qu'est-ce qui a résisté — et pourquoi ?
3. Y a-t-il un signal qui demande d'ajuster le séquençage de la Phase Standard ?
4. Quel est l'unique goulot à traiter en priorité en Q3 ?

---

*Version 1.0 — Mai 2026*
*Business Ascension™ — Roadmap Business*
