> **CONTEXTE BA™ (ajouté 2026-06-12)** : Ce vault obsidian-mind vit dans `08-SECOND-CERVEAU/` du repo Business Ascension™ — ce n'est PAS la racine du projet. Le CLAUDE.md racine de BA prévaut en cas de conflit. Tooling non installé ici : QMD, MCP qmd, defuddle, hooks de session (utiliser Grep/Glob/Read à la place). Les commandes /om-* sont installées dans .claude/commands/ du repo BA avec les chemins adaptés.
...
|--------|---------|-----------|
| `08-SECOND-CERVEAU/Home.md` | **Vault entry point** -- embedded Base views, quick links | Open this first |
...
| `08-SECOND-CERVEAU/bases/` | **All Bases centralized** -- dynamic views for navigation | `Work Dashboard`, `Incidents`, `People Directory`, `1-1 History`, `Review Evidence`, `Competency Map`, `Templates` |
| `08-SECOND-CERVEAU/work/` | Work notes index | `Index.md` (detailed MOC) |
| `08-SECOND-CERVEAU/work/active/` | **Current projects only** (1-3 files) | Move here when starting, move to archive when done |
| `08-SECOND-CERVEAU/work/archive/YYYY/` | Completed work organized by year | Grows over time |
| `08-SECOND-CERVEAU/work/incidents/` | Incident docs (main note + RCA + deep dive + drafts) | Per-incident grouping |
| `08-SECOND-CERVEAU/work/1-1/` | 1:1 meeting notes (accumulate weekly) | Named `<Person> YYYY-MM-DD.md` |
| `08-SECOND-CERVEAU/work/meetings/` | **Meeting notes inbox** -- staging area for raw exports, processed by `/om-intake` | Drop files, run `/om-intake` |
| `08-SECOND-CERVEAU/perf/` | Performance framework, brag doc | `Brag Doc.md` (index) |
| `08-SECOND-CERVEAU/perf/brag/` | Quarterly brag notes | One per quarter, e.g. `Q1 2025.md` |
| `08-SECOND-CERVEAU/perf/competencies/` | Atomic competency notes (link targets) | One note per competency |
| `08-SECOND-CERVEAU/perf/evidence/` | PR deep scans, data extracts for reviews | Named `<Person> PRs - <Period>.md` |
| `08-SECOND-CERVEAU/perf/<cycle>/` | Review cycle briefs + artifacts | Review briefs (private, manager, peer) |
| `08-SECOND-CERVEAU/brain/` | Claude's operational knowledge | `Memories.md`, `Key Decisions.md`, `Patterns.md`, `Gotchas.md`, `Skills.md`, `North Star.md` |
| `08-SECOND-CERVEAU/org/` | Organizational knowledge index | `People & Context.md` (MOC) |
| `08-SECOND-CERVEAU/org/people/` | Atomic person notes | One note per person |
| `08-SECOND-CERVEAU/org/teams/` | Team notes as graph nodes | One note per team |
| `reference/` | Codebase knowledge, architecture maps | Flow docs, architecture docs |
| `08-SECOND-CERVEAU/thinking/` | Scratchpad for drafts and reasoning | Named `YYYY-MM-DD-topic.md` |
| `08-SECOND-CERVEAU/templates/` | Obsidian templates | `Work Note.md`, `Decision Record.md`, etc. |
...
The `SessionStart` hook automatically injects rich context: vault file listing, North Star goals, active work, recent git changes, open tasks (aggregated from `08-SECOND-CERVEAU/work/active/` and the vault root, excluding infrastructure files), and triggers a QMD re-index. Most context is already loaded -- you don't need to manually read files.
...
1. Read `08-SECOND-CERVEAU/Home.md` -- vault entry point with embedded dashboards
2. Read `08-SECOND-CERVEAU/brain/North Star.md` -- ground suggestions in current goals
3. Check `08-SECOND-CERVEAU/work/Index.md` -- see active projects and recent notes
4. Scan `08-SECOND-CERVEAU/brain/Memories.md` -- index of memory topics, then read relevant topic notes
...
1. **Archive completed projects**: `git mv` from `08-SECOND-CERVEAU/work/active/` to `08-SECOND-CERVEAU/work/archive/YYYY/`, update `status: completed` (or use `/om-project-archive`)
2. Update `08-SECOND-CERVEAU/work/Index.md` if new notes or decisions were created
3. Update the relevant brain topic note (`08-SECOND-CERVEAU/brain/Key Decisions.md`, `08-SECOND-CERVEAU/brain/Patterns.md`, `08-SECOND-CERVEAU/brain/Gotchas.md`) with key learnings
4. Update `08-SECOND-CERVEAU/org/People & Context.md` if org knowledge changed
5. Update `08-SECOND-CERVEAU/perf/Brag Doc.md` if wins or impact were achieved
6. Offer to update `08-SECOND-CERVEAU/brain/North Star.md` if goals shifted or new focus emerged
...
Use `08-SECOND-CERVEAU/thinking/` for drafts, reasoning, and analysis before writing final notes. **Thinking notes are scratchpads, not storage.** They exist to help you reason -- once the reasoning produces durable knowledge, promote it to proper notes and delete the scratchpad.

1. Create a thinking note: `08-SECOND-CERVEAU/thinking/YYYY-MM-DD-descriptive-name.md`
...
2. **Use templates** from `08-SECOND-CERVEAU/templates/`. Fill `{{placeholders}}` with real values.
3. **Place files correctly**:
   - **Active** work notes, decisions, peer review prep -- `08-SECOND-CERVEAU/work/active/`
   - **Completed** work notes -- `08-SECOND-CERVEAU/work/archive/YYYY/` (by year)
   - Incident docs -- `08-SECOND-CERVEAU/work/incidents/`
   - 1:1 meeting notes -- `08-SECOND-CERVEAU/work/1-1/`
   - Performance content -- `08-SECOND-CERVEAU/perf/` (cycle subfolder for review briefs)
   - PR evidence -- `08-SECOND-CERVEAU/perf/evidence/`
   - Competency definitions -- `08-SECOND-CERVEAU/perf/competencies/`
   - People -- `08-SECOND-CERVEAU/org/people/`
   - Teams -- `08-SECOND-CERVEAU/org/teams/`
   - Claude operational context -- `08-SECOND-CERVEAU/brain/`
   - Codebase knowledge -- `reference/`
   - Drafts -- `08-SECOND-CERVEAU/thinking/`
   - Vault root: `08-SECOND-CERVEAU/Home.md`, `CLAUDE.md`, `AGENTS.md`, `GEMINI.md`, `vault-manifest.json`, `CHANGELOG.md`, `CONTRIBUTING.md`, `README.md`, `LICENSE`, `.gitignore`. No user notes at root.
...
|------|----------|--------|--------------|
| Work note | `08-SECOND-CERVEAU/work/active/` (then `archive/YYYY/` when done) | Descriptive title | Context, What/Why, Links, Related |
| Incident | `08-SECOND-CERVEAU/work/incidents/` | Ticket number or descriptive title | Context, Root Cause, Timeline, Impact, Analysis, Related |
| 1:1 note | `08-SECOND-CERVEAU/work/1-1/` | `<Person> YYYY-MM-DD.md` | Key Takeaways, Action Items, Quotes, What to Watch, Related |
| PR analysis | `08-SECOND-CERVEAU/perf/evidence/` | `<Person> PRs - <Period>.md` | PR Count, Projects, Quality, Growth, Full Table |
| Review brief | `08-SECOND-CERVEAU/perf/<cycle>/` | `<Cycle> Review Brief.md` | Arc, Impact, Competencies, Documentation Trail |
| Person note | `08-SECOND-CERVEAU/org/people/` | Full name | Role & Team, Relationship, Key Moments, Notes |
| Team note | `08-SECOND-CERVEAU/org/teams/` | Team name | Members, Scope, Interactions |
| Competency | `08-SECOND-CERVEAU/perf/competencies/` | Competency name | Definition, level criteria, Evidence (via backlinks) |
| Brain note | `08-SECOND-CERVEAU/brain/` | Topic name | Topic-specific content |
...
- **Person nodes** (08-SECOND-CERVEAU/org/people/): link to projects, teams, evidence. Receive backlinks from work notes.
...
- **Index -> Everything**: `08-SECOND-CERVEAU/work/Index.md` links to all work notes
...
- **`08-SECOND-CERVEAU/work/Index.md`** -- add to Active Projects or Recent Notes, move completed to Archive
- **`08-SECOND-CERVEAU/brain/Memories.md`** -- index of memory topics. Add new memories to the relevant topic note, not here.
- **`08-SECOND-CERVEAU/brain/Skills.md`** -- register vault-specific workflows and slash commands
- **`08-SECOND-CERVEAU/org/People & Context.md`** -- update when people, teams, or org structure changes
- **`08-SECOND-CERVEAU/perf/Brag Doc.md`** -- log wins with links to evidence, add new quarters as needed

### Decision Records

1. Create in `08-SECOND-CERVEAU/work/` using the Decision Record template
2. Link from the work note(s) that led to the decision
3. Add to the Decisions Log table in `08-SECOND-CERVEAU/work/Index.md`
4. If significant, note in `08-SECOND-CERVEAU/brain/Key Decisions.md`

### Wins & Achievements

When significant work is completed, add to `08-SECOND-CERVEAU/perf/Brag Doc.md` with links to the work note(s). Categorize under Impact, Technical Growth, Collaboration, or Feedback.

## North Star

`08-SECOND-CERVEAU/brain/North Star.md` is a living document of goals and focus areas.
...
| **Vault memories** | `08-SECOND-CERVEAU/brain/` topic notes | Git-tracked, Obsidian-browsable, linked. All durable knowledge lives here. |

When asked to "remember" something:
1. Find or create the appropriate `08-SECOND-CERVEAU/brain/` topic note (Gotchas, Patterns, Key Decisions, etc.)
2. Add the knowledge there with a wikilink to context
3. Update `08-SECOND-CERVEAU/brain/Memories.md` index if a new topic note was created
...
The SessionStart hook injects a **Brain Topics (read on demand)** index listing each `08-SECOND-CERVEAU/brain/` topic note with its description and an `(empty)` marker for stub notes. Treat that index as a menu:

- When the user's message touches a topic from the index (debugging → Gotchas, "how do we usually…" → Patterns, "why did we decide" → Key Decisions, "which command / slash" → Skills), query QMD **first** before answering — call `mcp__qmd__query` with a `query` argument describing the topic (or fall back to `qmd --index <name> query "<topic>"` if MCP is unavailable). The search covers the whole vault, so filter or prioritize results whose `file` path is under `08-SECOND-CERVEAU/brain/`. Do not assume the topic name alone scopes the search.
- If QMD is unavailable, read the specific `08-SECOND-CERVEAU/brain/` note directly with the Read tool. Don't load all of `08-SECOND-CERVEAU/brain/` — only the one(s) matching the topic.
...
- **Writing about a person?** -- `08-SECOND-CERVEAU/org/people/`
- **Writing about a team?** -- `08-SECOND-CERVEAU/org/teams/`
- **Writing about how the codebase works?** -- `08-SECOND-CERVEAU/brain/` (Patterns, Gotchas, Key Decisions)
- **Writing about what Claude should remember?** -- `08-SECOND-CERVEAU/brain/Memories.md` topic notes
- **Capturing a 1:1 meeting?** -- `08-SECOND-CERVEAU/work/1-1/`
- **Deep scanning PRs for review?** -- `08-SECOND-CERVEAU/perf/evidence/`
- **Creating review briefs?** -- `08-SECOND-CERVEAU/perf/<cycle>/`
- **Tracking active project work?** -- `08-SECOND-CERVEAU/work/active/`
- **Capturing an incident?** -- `08-SECOND-CERVEAU/work/incidents/` (use `/om-incident-capture`)
...
- **Project evidence** (PRs, technical decisions, delivery) -- goes to the relevant `08-SECOND-CERVEAU/work/` note
- **Review prep** (peer selection, manager strategy, brag framing) -- goes to review-related notes in `08-SECOND-CERVEAU/perf/` or `08-SECOND-CERVEAU/work/`
- **People dynamics** (feedback, relationships, career) -- goes to `08-SECOND-CERVEAU/org/people/` notes
...
| PreCompact | Before context compaction | Backs up session transcript to `08-SECOND-CERVEAU/thinking/session-logs/` |
...
- When asked to "remember" something, write to the relevant `08-SECOND-CERVEAU/brain/` topic note with a link to context. Never create memory files in `~/.claude/` -- they are not git-tracked.
